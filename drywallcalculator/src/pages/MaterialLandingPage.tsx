...
export default function MaterialLandingPage() {
  ...
  return (
    <main id="main-content" className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12" role="main">
      ...
      <DrywallCalculator
        initialIncludeCeiling={page.includeCeiling}
        heading={page.h1}
              ...
              <li key={guide.slug}>
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3">
                  <Link
                    ...
                    className="block text-sm font-medium hover:text-orange-700 dark:hover:text-orange-400 transition-colors"
                  >
                    {guide.title}
                  </Link>
                  {guide.toolPath && guide.toolLabel && (
                    <Link
                      to={guide.toolPath}
                      className="mt-1 block text-xs font-medium text-orange-600 dark:text-orange-400 hover:underline"
                    >
                      {guide.toolLabel} →
                    </Link>
                  )}
