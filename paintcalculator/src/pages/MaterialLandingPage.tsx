  ...
  return (
    <main id="main-content" className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12" role="main">
      ...
      {page.kind === 'wallpaper' ? (
        ...
        <PaintCalculator
          initialPaintType={page.paintType}
          initialSurface={page.surface}
          heading={page.h1}
          subheading={pageIntro ?? page.intro}
        />
      )}
              ...
              <li key={guide.slug}>
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3">
                  <Link
                    ...
                    className="block text-sm font-medium hover:text-blue-700 dark:hover:text-blue-400 transition-colors"
                  >
                    {guide.title}
                  </Link>
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {relatedPages.map((related) => (
              <li key={related.slug}>
                <Link
                  to={related.path}
                  className="block rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3 text-sm font-medium hover:border-blue-300 dark:hover:border-blue-700 transition-colors"
                >
