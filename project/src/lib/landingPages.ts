...
function buildSatoshiHubPage(currency: FiatCurrency): LandingPageDef {
  ...
  const description =
    currency === 'usd'
      ? 'Convert sats to USD at the live Bitcoin price. Free Satoshi calculator — updated every 60 seconds from CoinGecko.'
...
function buildSatoshiAmountPage(amount: number, currency: FiatCurrency): LandingPageDef {
  ...
  return {
    ...
    amount,
    currency,
    title: `${titleAmount} to ${label} — Live Price Today`,
    description: `How much is ${formatted} sats in ${label}? See the live ${label} value at today's Bitcoin price. Free calculator — updated every 60 seconds.`,
