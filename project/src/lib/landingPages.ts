...
function buildSatoshiHubPage(currency: FiatCurrency): LandingPageDef {
  ...
  const title =
    currency === 'usd'
      ? 'Satoshi to USD Converter — Live Sats Value Today (2026)'
      : currency === 'eur'
...
function satoshiAmountShortLabel(amount: number): string {
  ...
  return `${formatSatoshiAmount(amount)} sats`;
}
...
function buildSatoshiAmountPage(amount: number, currency: FiatCurrency): LandingPageDef {
  ...
  const short = satoshiAmountShortLabel(amount);

  const title =
    currency === 'usd'
      ? `${formatted} Satoshis to USD — How Much Is ${short}?`
      : `${formatted} Satoshi to ${label} — Live Price Today`;

  const description =
    currency === 'usd'
      ? `How much is ${formatted} Satoshis (${short}) in USD today? See the live dollar value at the current Bitcoin price. Free calculator — updated every 60 seconds.`
