...
export default function BitcoinFeeCalculatorPage() {
  ...
  const faqItems = useMemo(
    () => [
      ...
      {
        question: 'What if fees spike after I already broadcast?',
        answer:
          'You cannot lower a fee that already confirmed. If the tx is still unconfirmed, some wallets support Replace-By-Fee (RBF) or Child-Pays-For-Parent — check your wallet docs before you send. Next time, model sat/vB here first and compare fee USD to your stack on [Satoshi to USD](/satoshi-to-usd) or [100k sats](/100000-satoshi-to-usd) so you know whether to wait or use Lightning.',
      },
    ],
    ...
    <main id="main-content" className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12" role="main">
      ...
      <section className="mb-10 prose prose-slate dark:prose-invert max-w-none">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8 shadow-sm space-y-6 text-slate-600 dark:text-slate-300 leading-relaxed">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mt-0 mb-3">
              After you broadcast: RBF is the backup, not the plan
            </h2>
            <p>
              Once a transaction confirms, you cannot claw back the fee. If it is still unconfirmed and your wallet
              supports Replace-By-Fee, you may bump sat/vB — but that only works when you enabled RBF before the first
              send. Prefer modeling tonight’s rate here first, then compare fee USD to your stack on{' '}
              <Link to="/satoshi-to-usd" className="text-orange-600 dark:text-orange-400 hover:underline">
                Satoshi to USD
              </Link>{' '}
              or{' '}
              ...
              so you know whether to wait or use Lightning.
            </p>
          </div>
        </div>
      </section>
